...
const AddProperty: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await api.post("/admin/create-room", updatedRoomData);
  ...
  const getCurrentLocation = () => {
    console.log("gerring cutten locaiton");
  ...
  const [admin, setAdmin] = useState<null | Owner>(null);
  ...
  const getOwner = async () => {
    try {
      const data = await api.get(`/admin/get-owner/${selectedOwner}`);
      if (data.status === 200) {
        setUser(data.data);
      }
    } catch (error) {}
  ...
  const getAllOwners = async () => {
    try {
      const data = await api.get("/admin/get-owners");
      console.log(data);
      setOwners(data.data);
    } catch (error) {}
  };
  ...
  useEffect(() => {
    if (user?.planType === "FREE") {
      ...
      return setFreePlan(true);
    } else if (!!user?.locations && user.planType === "PRO") {
    ...
    <div className="w-full p-2 lg:p-6 bg-white rounded-2xl shadow-lg border border-gray-200 pt-16">
      ...
      {freePlan && (
        <p className="text-red-500">
          *The Current User Is On Free Plan Only Admin Can Add Extra Properties
          for them
        </p>
      )}
      <CustomSearchDropdown
        owners={owners}
        onSelect={(ownerId) => setSelectedOwner(ownerId)}
      />
